...
class Zombie:
    ...
    def run_to_boy(self, r=0.5):
        ...
        if self.distance_less_than(self.x - (play_mode.boy.x - self.x), self.y - (play_mode.boy.y - self.y), self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        ...
        c1 = Condition('소년보다 공이 많은가?', self.is_ball_up)
        a4 = Action('소년한테 접근', self.move_to_boy)

        root = SEQ_chase_boy = Sequence('소년을 추적', c1, a4)

        c2 = Condition('소년보다 공이 적은가?', self.is_ball_down)
        a5 = Action('소년한테 도망', self.run_to_boy)
        root = SEQ_run_boy = Sequence('소년에게서 도망', c2, a5)

        c3 = Condition('소년이 근처에 있는가?', self.is_boy_nearby, 7)
        root = SEL_chase_or_run = Selector('추적 또는 도망', SEQ_chase_boy,SEQ_run_boy)
        root = SEQ_near_boy = Sequence('소년이 근처에 있는가?', c3, SEL_chase_or_run)

        root = SEL_near_or_not = Selector('소년이 주변에 존재 또는 비존재', SEQ_near_boy, SEQ_wander)
